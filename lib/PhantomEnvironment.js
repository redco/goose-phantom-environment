...
const _ = require('lodash'),
  Environment = require('goose-parser/lib/Environment'),
...
const defaultOptions = {
  ...
  cookiesFile: null,
  webSecurity: false,
  phantomPath: path.join(__dirname, '../../node_modules/phantomjs-prebuilt/bin/'),
...
class PhantomEnvironment extends Environment {
    /**
  ...
  snapshot(fileName) {
    ...
    const screenShotFilePath = path.join(options.snapshotDir, this._getHostName(this._url));
  ...
  _getElementPosition(selector) {
    return this.evaluateJs(selector, /* @covignore */ (selector) => {
  ...
  _rotateProxy() {
    ...
    if (Array.isArray(proxy)) {
      ...
      const promise = (typeof this._options.proxyRotator === 'function') ?
  ...
  _extractRedirectUrl(resource) {
    ...
    if (resource.redirectUrl) {
      ...
      const locationHeader = (resource.headers || []).find(header => header.name && header.name.toLowerCase() === 'location');
